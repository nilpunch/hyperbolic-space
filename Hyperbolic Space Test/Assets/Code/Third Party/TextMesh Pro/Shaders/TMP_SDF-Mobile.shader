...
SubShader {
	...
	Pass {
		...
		pixel_t VertShader(vertex_t input)
		{
            ...
            float4 modelScaleRotation = mul(unity_ObjectToWorld, vert) - localPlanarOffset;
			
            // Normalization step
            if (_GlobalCurvature < 0)
            {
                localPlanarOffset = fromKleinToPoincare(localPlanarOffset, _GlobalCurvature);
            }
            modelScaleRotation = fromKleinToPoincare(modelScaleRotation, _GlobalCurvature);
			
            // GyroVector Transformation
            float4 vertexPosition = mobiusAdd(localPlanarOffset, modelScaleRotation, _GlobalCurvature);
            ...
            vertexPosition = mobiusAdd(globalPlanarOffset, vertexPosition, _GlobalCurvature);
            vertexPosition = mobiusAdd(globalHeightOffset,  vertexPosition, _GlobalCurvature);
			
            // Transform Poincare coordinates to the coordinates of another model (by factor)
            vertexPosition = lerp(vertexPosition, fromPoincareToKlein(vertexPosition, _GlobalCurvature), _GlobalBeltramiKlein);
			
            // View Projection Matrix Transform
            vertexPosition = mul(UNITY_MATRIX_VP, vertexPosition);

			// Instead of float4 vPosition = UnityObjectToClipPos(vert);
			float4 vPosition = vertexPosition;
